...
**Long documents.** `laya-multilingual` accepts up to 8,192 tokens with `max_len`. Measured accuracy and time by document length, reproducible with [`research/scripts/bench_long_context.py`](https://github.com/NandhaKishorM/laya/blob/main/research/scripts/bench_long_context.py):

<p align="center">
  <img src="https://raw.githubusercontent.com/NandhaKishorM/laya/main/assets/long_context_table.png" alt="laya-multilingual long-document accuracy: with the request after 2,000 to 4,000 tokens of other text, 17 to 18 of 20 correct at a higher limit against 7 of 20 at the default 1,024; unstable beyond about 4,000 tokens" width="100%" />
</p>

## Quickstart

> **Long documents: `laya-multilingual` accepts up to 8,192 tokens and is reliable up to about 4,000.** It ships with a 1,024-token limit, which cuts a long document off; raise it per call:
>
> ```python
> result = router.predict(long_document, questions, model="multilingual", max_len=4096)
> ```
>
> In the table above, with the request placed after 2,000 to 4,000 tokens of other text, 17 to 18 of 20 requests were answered correctly at the higher limit, against 7 of 20 at the default. Beyond about 4,000 tokens results become unstable (8 to 17 of 20), probably because the checkpoint was trained with its 1,024-token limit, so treat `max_len=8192` as experimental. Short inputs give identical answers at any limit. Speed follows the input's real length, not the limit: short inputs are unchanged, and a 4,000-token input takes about 1.7 s on an Apple GPU. Name the checkpoint with `model="multilingual"`, since long mostly-English text would otherwise route to the English checkpoint.
...
* **Measured long-document table.** Accuracy and time for `laya-multilingual` by document length, from a reproducible benchmark: reliable up to about 4,000 tokens with a higher `max_len`, and experimental beyond that. This corrects the earlier note, which said accuracy held all the way to 8,192.
